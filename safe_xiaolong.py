...
class SAFE:
    ...
    def calculateHypergeomThreads(self, N_in_neighborhood_in_group, N, N_in_group, N_in_neighborhood, threads=28):
        '''calculate p-value with multiple cpus
        '''
        from multiprocessing import Pool
        N_in_neighborhood_in_group1 = N_in_neighborhood_in_group - 1
        
        pool = Pool(threads)
        results = pool.starmap(hypergeom.sf, zip(N_in_neighborhood_in_group1,N,N_in_group,N_in_neighborhood))
        
        pool.close()
        pool.join()
        results1 = np.vstack(results)
        return results1
    ...
    def compute_pvalues_by_hypergeom(self, **kwargs):
        ...
        if kwargs:
            if 'processes' in kwargs:
                threads = int(kwargs['processes'])

        if threads <= 1:
            print('use single thread to calculate p-value')
            self.pvalues_pos = hypergeom.sf(N_in_neighborhood_in_group - 1, N, N_in_group, N_in_neighborhood)
        else:
            print('use', threads, 'threads to calculate p-value')
            self.pvalues_pos = self.calculateHypergeomThreads(N_in_neighborhood_in_group, N, N_in_group, N_in_neighborhood,threads)
