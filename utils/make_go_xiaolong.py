...
def make_go_matrix(path_to_obo='', path_to_annotations='', go_branch='p'):
    ...
    if path_to_annotations.endswith('.gaf') or path_to_annotations.endswith('.gaf.gz'):
        print('input annotation file is a gaf file')
        go_annotations = read_annotations(path_to_annotations)
    else:
        print('input annotation file maybe a dataframe from ensembl biomart')
        go_annotations = read_annotations_biomart(path_to_annotations)
    ...
    saveMatrix(df_matrix=go_matrix, outfile=txt_path)

def saveDF(tdf):
    tdf = tdf.astype(str)
    return tdf.to_csv(sep='\t',header=None)

def saveMatrix(df_matrix, outfile):
    '''
    save matrix with multiple threads to speed up
    '''
    threads = os.cpu_count()
    
    ls_df = np.array_split(df_matrix, threads)
    header = df_matrix.head(0)
    header = header.to_csv(sep='\t')
    from multiprocessing import Pool
    pool = Pool(threads)
    results = pool.map(saveDF,ls_df)
    pool.close()
    pool.join()
...
def read_annotations_biomart(path_to_annotations):
    ...
    :return: data frame
    """
    print("Reading the gene-to-GO term annotations from biomart...")
    go_annotations = pd.read_csv(path_to_annotations, delimiter='\t', comment="!")
    dc_newname = {'Gene stable ID':'gene_id', 'GO term accession':'GO_id', 'GO domain':'GO_domain', 'GO term name':'GO_name'}
    go_annotations.columns = [dc_newname[e] if e in dc_newname else e for e in go_annotations.columns ]
    # remove lines with empty 'GO_id'
    go_annotations = go_annotations[go_annotations['GO_id'].notnull()].copy()
    go_annotations.index = go_annotations['gene_id']
    return go_annotations
...
def make_locus2term(go_graph, go_annotations):
    ...
    for go_term in tqdm(go_terms):
        ...
        go_term_loci = sorted(go_annotations.loc[go_annotations['GO_id'] == go_term, 'gene_id'].unique().tolist())
...
description = '''Generate a new matrix of genes (loci) to GO term associations.
the input annotation file must have 'Gene stable ID' or 'gene_id', 'GO term accession' or 'GO_id' column
'''
